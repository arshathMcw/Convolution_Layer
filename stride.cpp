...
using namespace std;
int main(){
    int input_height = 9, input_width = 9,in_channel = 3,out_channel = 2 , kernel_height = 3, kernel_width = 3 ;
    int stridex = 1,stridey = 3;
    int output_height = ((input_height - kernel_height) / stridex) + 1,output_width = ((input_width - kernel_width) / stridey) + 1;
                ...
                for(int cch = 0;cch < in_channel;cch++){
                    for(int rr = 0;rr < kernel_height;rr++){
                        for(int cc = 0;cc < kernel_width;cc++){
                            res += (input[cch][(r * stridex)+rr][(c * stridey)+cc] * filter[ch][cch][rr][cc]);
                        }
                    }
                }
                output[ch][r][c] = res;
